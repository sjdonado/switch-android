...
open class UsersPlaceService (activity: Activity) : MainService("/users-places", activity) {
    ...
    fun accept(userId: String, placeId: String, callback: (err: Boolean, response: JSONObject) -> Unit) {
    ...
    fun reject(userId: String, placeId: String, callback: (err: Boolean, response: JSONObject) -> Unit) {
        val jsonObject = JSONObject()
        jsonObject.put("userId", userId)
        jsonObject.put("placeId", placeId)
    ...
    fun qualify(id: String, value: Float, comment: String, callback: (err: Boolean, response: JSONObject) -> Unit) {
        val jsonObject = JSONObject()
    ...
    fun remove(id: String, callback: (err: Boolean, response: JSONObject) -> Unit) {
